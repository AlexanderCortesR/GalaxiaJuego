 ...
 * @author Alexander
 */
public class ColisionBN implements Pintar {

    private NaveJugador naveJugador;
    private NaveEnemigo naveEnemigo;

    private ArrayList<Rectangle> choques = new ArrayList<>();
    private Image imagen = null;
    private MediaTracker tracker;

    public ColisionBN(Component componente, NaveJugador naveJugador, NaveEnemigo naveEnemigo) {
        
        this.naveJugador = naveJugador;
        this.naveEnemigo = naveEnemigo;
    ...
    public void colisiones() {
        ...
        if (naveJugador.getBalas().size() > 0) {
            ...
            while (iter0.hasNext()) {
                ...
                Iterator iter = naveEnemigo.getNaveEnemigo().iterator();

                while (iter.hasNext()) {
                    Rectangle rectanguloDos = ((NaveEnemigoAux) iter.next()).getRec();
        ...
        if (naveEnemigo.getBalas().size() > 0) {
            Iterator iter0 = naveEnemigo.getBalas().iterator();
